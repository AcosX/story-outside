...
- ClickUp 08 P1.6：未配置数据库时 runtime / sessionService 使用内存 repository，只提供本进程 recover；配置 MariaDB 后，server 在启动时 hydrate projection，`recoverSession` 可跨进程恢复 canonical history、revision、cursor、pending 与 compact state，且仍不调用 provider、不重放、不追加。
