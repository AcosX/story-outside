...
// follow/relationship surface (ClickUp 16.3 rebuild).
//
// Hard rules:
...
//   * Mock auth looks up `X-Mock-User-uuid` against this list. There is no
//     way to inject an identity through the request body (the request
//     handlers reject identity-shaped keys like `user_ref`, `user_uuid`,
//     `identity` outright — see tests/ecosystemFollowingRebuilt.test.mjs).
...
export function listMockFollowingIdentities() {
  return Object.freeze(Object.values(MOCK_FOLLOWING_FIXTURE_USERS).map((id) => Object.freeze({ ...id })));
}