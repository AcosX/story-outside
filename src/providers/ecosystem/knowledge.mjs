...
//      JSON.stringify({ story_version_uuid, community_profile_version, query_id })
 ...
 *                                                     or real).
 * @property {{ story_version_uuid: string, community_profile_version: string, query_id: string | null }} cache_key
 ...
 *
 * @param {{ story_version_uuid: string, community_profile_version: string, query_id?: string | null }} input
 * @returns {string}
 */
 ...
 * query_id?, limit?, force? })` that resolves with an
 ...
 *     (story_version_uuid, community_profile_version, query_id) tuples
 ...
 * @returns {{
 *   match: (input: { story_version_uuid: string, community_profile_version: string, query_id?: string | null, limit?: number, force?: boolean }) => Promise<EcosystemKnowledgeResponse>,
...
export function createEcosystemKnowledgeProvider(opts = {}) {
  ...
  async function match(input) {
    ...
    const cacheKey = buildKnowledgeCacheKey({
      ...
      query_id: typeof input.query_id === 'string' && input.query_id ? input.query_id : null,
    });
