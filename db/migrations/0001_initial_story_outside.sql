-- Story Outside - initial MariaDB schema.
--
-- Compatibility: MariaDB 10.6+ / 11.x, InnoDB, utf8mb4.
-- Database: run this against a target database; db/schema.sql contains the
-- canonical fresh-install entrypoint that also creates the database.
--
-- Migration policy:
--   * 0001 is additive and re-runnable: CREATE TABLE IF NOT EXISTS,
--     DROP TRIGGER IF EXISTS, and INSERT IGNORE are safe to repeat.
--   * Do not edit an applied migration on a deployed environment. Future
--     structural changes must be new numbered files (0002_*.sql, ...).
--   * All DATETIME(6) columns are UTC. Apps must write UTC instants.
--   * This file never stores credentials, tokens, or app secrets.
