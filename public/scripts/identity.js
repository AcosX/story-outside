// public/scripts/identity.js — ClickUp 16.4 P1.v2 fix (2026-09-07).
//
...
(function () {
  ...
  // Strict semver for the community_profile_version field. The
  // hot.mjs orchestrator also enforces this server-side, but the
  // browser validates up front so a bad version never reaches the
  // network (ClickUp 16.4 P1.v2 contract: no silent "0 terms").
  ...
  function assertTripleShape(input) {
    ...
    if (!SEMVER_PATTERN.test(community_profile_version)) return null;
