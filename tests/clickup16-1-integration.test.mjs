...
test('P1.1 + P2 together: importStoryAndEnsureCache with profileRepository produces a mock-generated profile (default)', async () => {
  const { repository, profileRepository } = makeRepositoryAndProfileRepo();
  const version = await importCafeRainFirstTime(repository, profileRepository);
  // The mock path always tags as 'mock-generated' (the real-provider
  // switch is part of ClickUp 16.7+). The important regression
  // contract is: a profile MUST exist after the import; and the
  // provenance must be a value from PROFILE_SOURCES (no leaks).
  const profile = getCommunityProfile({
    profileRepository,
    story_version_uuid: version.version_uuid,
  });
  assert.ok(profile, 'profile must be readable after import');
  const allowed = ['mock-generated', 'mock-fixture', 'real-generated', 'manual'];
  assert.ok(
    allowed.includes(profile.source),
    `profile.source must be a known provenance (got '${profile.source}')`,
  );
});
