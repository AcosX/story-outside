...
const base={summary:'玩家与友人重逢。',ending:'重逢',original_difference:'选择不同。',key_choices:['留在车站'],character_outcomes:[{character:'我',fate:'找到友人'}]};
...
function project(payload){
  const session={story_uuid:'story',story_version_uuid:'version',cache_uuid:'cache',history:[{event_seq:2,event_type:'player_input',payload:{text:'我留在车站等她'}}],finish_envelope:{tool_call:{name:'finish_story',payload}},requestIds:new Map()};
  const repository={sessionState:{sessions:new Map([[sessionUuid,session]])},findStoryByUuid:()=>({slug:'work'}),findVersion:()=>({version_no:1,content_payload:{beats:[{text:original}]}}),findOpeningCacheByUuid:()=>({content_payload:{events:[]}})};
