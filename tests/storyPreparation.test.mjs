...
try {
  ...
  for (const version of ['story-preparation-v1', 'story-preparation-v2', 'story-preparation-v3']) await writeAICache(config, 'story-' + cacheHash({version,model:config.model,id:source.id,title:source.title,beats:source.beats}), {roles:[{id:'traveler',label:'陈远',mood:'旅人'}],first_person_role_id:'traveler',opening_events:[{type:'narration',text:'旅人走进了房间。'}]});
  const fetchImpl=async(_url, options)=>{calls++;
    ...
    assert.match(prompt, /不得提及任何角色私有的秘密、叮嘱、记忆、认知或内心/);return {ok:true,json:async()=>({choices:[{message:{content:JSON.stringify({roles:[{id:'traveler',label:'陈远',mood:'旅人'}],first_person_role_id:'traveler',opening_events:[{type:'narration',text:'雨落在窗前。',story_progress:0.08}]})}}]})};};
