...
也兼容分项配置的 `STORY_OUTSIDE_DB_HOST` / `STORY_OUTSIDE_DB_PORT` / `STORY_OUTSIDE_DB_NAME` / `STORY_OUTSIDE_DB_USER` / `STORY_OUTSIDE_DB_PASSWORD` 分项配置；同时设置时优先使用 `STORY_OUTSIDE_DATABASE_URL`。
